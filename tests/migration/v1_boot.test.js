...
describe('Boot Sequence: Model v0.1 DB -> Model v1.0 Engine', () => {
  ...
  it('safely recomputes v1.0 stats from raw historical logs and configs', async () => {
    ...
    const historicalLogs = [
      { id: 'log-1', axis: 'wisdom', type: 'journal_entry', date: '2025-01-01' },
    ...
    const axisConfigs = [
      { axis: 'wisdom', hasConsistencyTerm: false, expectedPerWeek: 2, paused: false },
    ...
    // 2. Hydration check
    expect(hydratedQuests.find(q => q.id === 'q-wisdom-journal').currentValue).toBe(1); // 1 journal entry = 1 pt
    ...
    expect(() => {
      ...
      expect(Number.isNaN(stats.wisdom)).toBe(false);
